...
Results <- list(smc = smc.df, modeled = X.hat.smc,
                error = X.error.smc, corrected = corrected,
                ncomps = ncomps)
class(Results) <- "smc"
Results
}
...
plot.smc <- function(x, variables = "all", ...) {
  ...
  print(with(df, ggplot(df, aes(smc, Variables)) +
    ...
    geom_point(aes(pch = Significant), size = 3) +
    ggtitle(paste("sMC Results for ncomps = ", x$ncomps, "\nCorrected = ", x$corrected)) +
    xlab("F-statistics") +
